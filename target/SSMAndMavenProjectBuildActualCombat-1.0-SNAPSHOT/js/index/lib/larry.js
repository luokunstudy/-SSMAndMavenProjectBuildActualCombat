 ...
 */
layui.define(
		[ 'jquery', 'layer', 'element', 'common' ],
		function(exports) {
			var $ = layui.jquery, layer = layui.layer, element = layui.element, common = layui.common, cacheName = 'navbarCache';
			var Navbar = function() {
				/**
				 *  默认配置
				 */
			...
			Navbar.prototype.render = function() {
						...
						if (cacheNav.larry === undefined) {
							$.ajax({
								...
								error : function(xhr, status, error) {
									common.cmsError('"LarryCMS error"' + error,
											'Nav缓存开启错误！');
								},
								complete : function() {
									_that.config.elem = $container;
				...
				/*if (_config.spreadOne) {
					$container.find('li.layui-nav-item').each(
						function() {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if (data[i].children !== undefined && data[i].children !== null	&& data[i].children.length > 0) {
						...
						for (var j = 0; j < data[i].children.length; j++) {
							ulHtml += '<dd>';
							ulHtml += '<a href="' + data[i].children[j].href + '" target="main">';
